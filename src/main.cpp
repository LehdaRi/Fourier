...
#define EP10M1 22025.465794806718 // e^10 - 1
...
inline double toLogScale(double x) {
    return 0.1*std::log(x*EP10M1+1.0);
}

inline double toNormalScale(double x) {
    return (std::exp(10.0*x)-1)/EP10M1;
}

void imageToSpectra(const std::string& srcFileName,
                    const std::string& ampSpectrumFileName,
                    const std::string& phaseSpectrumFileName) {
    printf("Loading image...\n");
    sf::Image srcImg;
    srcImg.loadFromFile(srcFileName);
    ...
    for (auto y=0u; y<srcImgYSize; ++y) {
        for (auto x=0u; x<srcImgXSize; ++x) {
            ...
            double r = 255*toLogScale(ampSpectrum[0][x+y*srcImgXSize]/255.0);
            if (r<0.0) r = 0.0;
            double g = 255*toLogScale(ampSpectrum[1][x+y*srcImgXSize]/255.0);
            if (g<0.0) g = 0.0;
            double b = 255*toLogScale(ampSpectrum[2][x+y*srcImgXSize]/255.0);
            if (b<0.0) b = 0.0;
            double a = 255.0;
            //double a = 255*toLogScale(ampSpectrum[3][x+y*srcImgXSize]/255.0);
            //if (a<0.0) a = 0.0;
    ...
    ampSpectrumImg.saveToFile(ampSpectrumFileName);
    phaseSpectrumImg.saveToFile(phaseSpectrumFileName);
}

void spectraToImage(const std::string& ampSpectrumFileName,
                    const std::string& phaseSpectrumFileName,
                    const std::string& destFileName) {
    printf("Inverse transforming from spectra...\n");
    sf::Image ampSpectrumImg, phaseSpectrumImg;
    ampSpectrumImg.loadFromFile(ampSpectrumFileName);
    phaseSpectrumImg.loadFromFile(phaseSpectrumFileName);
    ...
    for (auto y=0u; y<spectraImgYSize; ++y) {
        for (auto x=0u; x<spectraImgXSize; ++x) {
            ...
            //for (auto i=0u; i<4; ++i) {
            for (auto i=0u; i<3; ++i) {
                double amp = 255.0*toNormalScale(ampSpectrumImgData[(xx+yy*spectraImgXSize)*4+i]/255.0);
    ...
    invImg.saveToFile(destFileName);
}

int main(void) {
    std::ifstream toSpectraFiles("toSpectra.txt");
    std::ifstream toImageFiles("toImage.txt");

    std::string line;
    while (std::getline(toSpectraFiles, line))
    {
        if (line.size() == 0)
            continue;
        ...
        std::string::size_type pathPos = line.find_last_of('/');
        pathPos++;
        std::string::size_type extPos = line.find('.');
        if (pathPos != std::string::npos && extPos != std::string::npos) {
            base = line.substr(pathPos, extPos-pathPos);
            ext = line.substr(extPos, std::string::npos);
        }

        std::stringstream ampSpectrumFileName, phaseSpectrumFileName;
        ampSpectrumFileName << "res/toSpectra/" <<  base << "_ampSpectrum" << ext;
        phaseSpectrumFileName << "res/toSpectra/" << base << "_phaseSpectrum" << ext;

        if (access(ampSpectrumFileName.str().c_str(), F_OK) == -1 &&
            access(phaseSpectrumFileName.str().c_str(), F_OK) == -1) {
            printf("Forming spectra from %s...\n", line.c_str());
            imageToSpectra(line, ampSpectrumFileName.str(), phaseSpectrumFileName.str());
        }
    }

    while (std::getline(toImageFiles, line))
    {
        std::string base;
        ...
        if (pathPos != std::string::npos) {
            base = line.substr(pathPos, std::string::npos);
        }

        std::stringstream ampSpectrumFileName, phaseSpectrumFileName, destFileName;
        ampSpectrumFileName << line << "_ampSpectrum.png";
        phaseSpectrumFileName << line << "_phaseSpectrum.png";
        destFileName << "res/toImage/" << base << ".png";

        if (access(destFileName.str().c_str(), F_OK) == -1) {
            printf("Forming image from %s and %s...\n", ampSpectrumFileName.str().c_str(), phaseSpectrumFileName.str().c_str());
            spectraToImage(ampSpectrumFileName.str(), phaseSpectrumFileName.str(), destFileName.str());
        }
    }
}

//  Some code temporarly moved out of the way
